...
def get_logo_width_inches(logo_path: str, max_width_inches: float = 2.65, max_height_inches: float = 0.42, min_width_inches: float = 1.35) -> float:
    """Auto-scale logo width so it fits a short banner row and stays visually balanced."""
    try:
        ...
        return max(min_width_inches, width_inches)
    except Exception:
        return 2.35
...
def build_policy_document(data: dict, output_path: str, logo_path: str | None = None):
    ...
    if logo_path and os.path.exists(logo_path):
        ...
        run.add_picture(logo_path, width=Inches(get_logo_width_inches(logo_path)))
    else:
